...
kill_orphaned_pgrp(struct task_struct *tsk, struct task_struct *parent)
{
    ...
    //PR946492 mtk add to avoid zygote orphaned process group start
    ...
    //PR946492 mtk add to avoid zygote orphaned process group
    if (pgtask != NULL){
        if (!strncmp("zygote", pgtask->group_leader->comm, TASK_COMM_LEN)){
            avoid_zygote = 1;
        }

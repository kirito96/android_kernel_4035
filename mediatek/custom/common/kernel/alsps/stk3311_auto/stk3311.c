...
static u16 stk_prox_mean_ori = 0;
static u16 stk_offset_val = 0;
static u16 stk_thd_low = 0;
static u16 stk_thd_hi = 0;
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable)
{
	...
	//	obj->psi = 0xFFFF;	
		if (!(obj->psi_set))
		{
			...
			stk_thd_low = obj->ps_low_thd_boot;
		       stk_thd_hi = obj->ps_high_thd_boot;
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	stk_prox_mean_ori = obj->ps_stat_data[1];
	stk_thd_low = obj->ps_low_thd_boot;
	stk_thd_hi = obj->ps_high_thd_boot;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(diff > STK_MAX_MIN_DIFF)
	{
		...
		stk_thd_low = obj->psi + STK_LT_N_CT;
		stk_thd_hi = obj->psi + STK_HT_N_CT;
...
static ssize_t stk_show_low_threshold(struct device_driver *ddri, char *buf)
{
	int len = 0;
	long err;
...
static ssize_t stk_show_high_threshold(struct device_driver *ddri, char *buf)
{
	int len = 0;
	long err;
...
static ssize_t stk_show_offset(struct device_driver *ddri, char *buf)
{
	int len = 0;
	long err;
...
static ssize_t stk_show_originalPS(struct device_driver *ddri, char *buf)
{
	int len = 0;
	long err;
...
static ssize_t stk_show_style(struct device_driver *ddri, char *buf)
{
	int len = 0;
	long err;


	    len = snprintf(buf, PAGE_SIZE, "STK\n");
	return len;
}
/*----------------------------------------------------------------------------*/
...
#endif
static DRIVER_ATTR(low_threshold,    S_IWUSR | S_IRUGO, stk_show_low_threshold,  NULL);
static DRIVER_ATTR(high_threshold,    S_IWUSR | S_IRUGO, stk_show_high_threshold,  NULL);
static DRIVER_ATTR(offset,    S_IWUSR | S_IRUGO, stk_show_offset,  NULL);
static DRIVER_ATTR(originalPS,    S_IWUSR | S_IRUGO, stk_show_originalPS,  NULL);
static DRIVER_ATTR(style,    S_IWUSR | S_IRUGO, stk_show_style,  NULL);
/*----------------------------------------------------------------------------*/
